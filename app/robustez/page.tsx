...
type Integrity = {
  ...
  sinais_total: number;
  entradas_antes_do_fechamento: number;
  saidas_antes_da_entrada: number;
  fechados_incompletos: number;
...
type NewsRun = {
  ...
  status: string;
  artigos_recebidos: number;
  eventos_novos: number;
  eventos_repetidos: number;
  reacoes_processadas: number;
  falhas: number;
  detalhes: unknown;
  erro: string | null;
};
type NewsEvent = {
  ...
  url: string;
  categoria: string;
  direcao_esperada: string;
  confianca_pct: number;
  publicado_em: string;
  dominio_fonte: string | null;
  novidade: string;
};
type Impact = {
  categoria: string;
  ...
  reacoes: number;
  retorno_medio_excesso_pct: number | null;
  direcao_confirmada_pct: number | null;
  impactos_altos: number;
};
...
function Card({ children }: { children: React.ReactNode }) {
  return (
    <section style={{ background: S.panel, border: `1px solid ${S.border}`, borderRadius: 12, padding: 18 }}>
      {children}
    </section>
  );
}
...
function Metric({ label, value, detail, color }: { label: string; value: string; detail?: string; color?: string }) {
  return (
    <div style={{ background: S.soft, border: `1px solid ${S.border}`, borderRadius: 10, padding: 14, minWidth: 150, flex: '1 1 150px' }}>
...
function statusColor(status: string) {
  ...
  if (status === 'executando') return S.blue;
  return S.red;
}
...
function providerLabel(provider: string) {
  const labels: Record<string, string> = {
    'gdelt-doc-2': 'GDELT',
    'coindesk-rss': 'CoinDesk RSS',
    'cointelegraph-rss': 'Cointelegraph RSS',
  ...
  const load = useCallback(async () => {
    ...
    const [validationResult, integrityResult, newsRunResult, eventResult, impactResult, reactionResult] = await Promise.all([
      ...
      supabase
        ...
        .select('id, provider, titulo, url, categoria, direcao_esperada, confianca_pct, publicado_em, dominio_fonte, novidade')
    ...
    setValidation(validationResult.data as Validation | null);
    setIntegrity(integrityResult.data as Integrity | null);
    setNewsRun(newsRunResult.data as NewsRun | null);
    setEvents((eventResult.data as NewsEvent[] | null) ?? []);
    setImpact((impactResult.data as Impact[] | null) ?? []);
      ...
      <header style={{ borderBottom: `1px solid ${S.border}`, background: S.panel, padding: '12px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 11, color: S.dim }}>notícias · contexto de mercado</div>
          </div>
        </div>
        ...
        <nav style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap', gap: 20, marginTop: 8, fontSize: 13 }}>
          ...
          {!session ? (
            ...
            <button onClick={() => supabase.auth.signOut()} style={{ background: 'transparent', border: 'none', color: S.red, fontSize: 13, cursor: 'pointer', padding: 0, fontFamily: 'inherit' }}>
              Sair
            </button>
          )}
        </nav>
          ...
          <>
            <Card>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                <div>
                  <div style={{ fontWeight: 700 }}>{config.nome} v{config.versao}</div>
                  <div style={{ color: S.dim, fontSize: 12, marginTop: 4 }}>Última validação: {date(validation?.iniciado_em)}</div>
                </div>
                <button onClick={() => void load()} style={{ background: S.blue, border: 0, color: 'white', padding: '8px 14px', borderRadius: 8, cursor: 'pointer' }}>
                  Atualizar
                </button>
              </div>
            </Card>
            ...
            <Card>
              <h2 style={{ fontSize: 15, marginTop: 0 }}>Prontidão estatística</h2>
              ...
              <div style={{ marginTop: 14, padding: 12, borderRadius: 8, background: ready ? 'rgba(63,178,111,.10)' : 'rgba(232,161,60,.08)', color: ready ? S.green : S.orange }}>
                {ready
                  ? 'Critérios automáticos mínimos atendidos. A decisão real ainda exige revisão humana e estabilidade por mais amostras.'
                  : 'Ainda não aprovado para dinheiro real. O painel continuará acumulando operações, cenários de estresse e Monte Carlo.'}
              </div>
            </Card>
            ...
            <Card>
              <h2 style={{ fontSize: 15, marginTop: 0 }}>Estudo prospectivo de notícias</h2>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                ...
                <Metric label="Reações programadas" value={fmt(reactionCount, 0)} />
                <Metric label="Reações concluídas no run" value={fmt(newsRun?.reacoes_processadas, 0)} />
                <Metric label="Falhas no run" value={fmt(newsRun?.falhas, 0)} color={(newsRun?.falhas ?? 0) > 0 ? S.orange : S.green} />
              </div>

              {providerEntries.length > 0 && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: 10, marginTop: 14 }}>
                  {providerEntries.map(([name, provider]) => (
                    <div key={name} style={{ background: S.soft, border: `1px solid ${S.border}`, borderRadius: 9, padding: 12 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                        <strong style={{ fontSize: 12 }}>{providerLabel(name)}</strong>
                        <span style={{ color: statusColor(String(provider.status)), fontSize: 11, fontWeight: 700 }}>{String(provider.status)}</span>
                      </div>
                      <div style={{ color: S.dim, fontSize: 11, marginTop: 6 }}>{fmt(provider.received, 0)} artigos nesta coleta</div>
              ...
              {newsRun?.erro && <div style={{ color: S.red, fontSize: 12, marginTop: 12 }}>{newsRun.erro}</div>}
              <p style={{ color: S.dim, fontSize: 12, lineHeight: 1.6, marginBottom: 0 }}>
                As notícias são somente contexto. Elas não criam, bloqueiam ou modificam operações da configuração congelada.
              </p>
            </Card>

            {impact.length > 0 && (
              <Card>
                <h2 style={{ fontSize: 15, marginTop: 0 }}>Impacto observado por categoria</h2>
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    <thead>
                      <tr>
                        {['Categoria', 'Direção textual', 'Horizonte', 'Amostra', 'Excesso médio', 'Direção confirmada', 'Impactos altos'].map((label) => (
                          <th key={label} style={{ textAlign: 'left', color: S.dim, padding: 8, borderBottom: `1px solid ${S.border}` }}>{label}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {impact.map((row, index) => (
                        <tr key={`${row.categoria}-${row.direcao_esperada}-${row.horizonte_minutos}-${index}`}>
                          <td style={{ padding: 8 }}>{row.categoria}</td>
                          <td style={{ padding: 8 }}>{row.direcao_esperada}</td>
                          <td style={{ padding: 8 }}>{row.horizonte_minutos} min</td>
                          <td style={{ padding: 8 }}>{row.reacoes}</td>
                          <td style={{ padding: 8 }}>{fmtPct(row.retorno_medio_excesso_pct)}</td>
                          <td style={{ padding: 8 }}>{fmtPct(row.direcao_confirmada_pct)}</td>
                          <td style={{ padding: 8 }}>{row.impactos_altos}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <Card>
              ...
              {events.length === 0 ? (
                <div style={{ color: S.dim }}>A coleta ainda não registrou eventos.</div>
              ) : (
                <div style={{ display: 'grid', gap: 10 }}>
                  {events.map((event) => (
                    <article key={event.id} style={{ background: S.soft, border: `1px solid ${S.border}`, borderRadius: 9, padding: 12 }}>
                      <a href={event.url} target="_blank" rel="noreferrer" style={{ color: S.text, fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>
                        {event.titulo}
                      </a>
                      <div style={{ color: S.dim, fontSize: 11, marginTop: 6 }}>
                        {providerLabel(event.provider)} · {event.categoria} · {event.direcao_esperada} · confiança {fmtPct(event.confianca_pct)} · {event.novidade} · {event.dominio_fonte ?? 'fonte não identificada'} · {date(event.publicado_em)}
                      </div>
                    </article>
                  ))}
                </div>
