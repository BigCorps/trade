...
function Card({ children }: { children: React.ReactNode }) {
  return <section style={{ background: S.panel, border: `1px solid ${S.border}`, borderRadius: 12, padding: 18 }}>{children}</section>;
}
function Metric({ label, value, detail, color }: { label: string; value: string; detail?: string; color?: string }) {
  return <div style={{ background: S.soft, border: `1px solid ${S.border}`, borderRadius: 10, padding: 14, minWidth: 150, flex: '1 1 150px' }}>
...
function Badge({ children, color }: { children: React.ReactNode; color: string }) {
  return <span style={{ color, border: `1px solid ${color}66`, background: `${color}16`, padding: '3px 7px', borderRadius: 999, fontSize: 10, fontWeight: 700 }}>{children}</span>;
}
      ...
      {!loading && config && <>
        <Card>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
            <div><strong>{config.nome} v{config.versao}</strong><div style={{ color: S.dim, fontSize: 12, marginTop: 4 }}>Última validação: {date(validation?.iniciado_em)}</div></div>
        ...
        <Card>
          ...
          {providerEntries.length > 0 && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(210px,1fr))', gap: 10, marginTop: 14 }}>
            {providerEntries.map(([name, provider]) => <div key={name} style={{ background: S.soft, border: `1px solid ${S.border}`, borderRadius: 9, padding: 12 }}>
        ...
        {exploratory.length > 0 && <Card>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
            <div><h2 style={{ fontSize: 15, margin: 0 }}>Cruzamentos exploratórios</h2><div style={{ color: S.dim, fontSize: 11, marginTop: 5 }}>Primeiro agrupados por evento; FDR Benjamini–Hochberg aplicado entre células testáveis.</div></div>
          ...
          <div style={{ overflowX: 'auto', marginTop: 12 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              ...
              <tbody>{exploratory.map((row, index) => <tr key={`${row.categoria}-${row.direcao_esperada}-${row.horizonte_minutos}-${index}`}>
                <td style={{ padding: 8 }}>{row.categoria}</td><td style={{ padding: 8 }}>{row.direcao_esperada}</td><td style={{ padding: 8 }}>{row.horizonte_minutos} min</td>
                <td style={{ padding: 8, fontWeight: 700 }}>{row.eventos_independentes}</td><td style={{ padding: 8, color: S.dim }}>{row.linhas_reacao}</td>
                <td style={{ padding: 8 }}>{fmtPct(row.retorno_medio_excesso_pct)}</td><td style={{ padding: 8 }}>{fmtPct(row.direcao_confirmada_pct)}</td>
                <td style={{ padding: 8 }}>{row.p_value === null ? '—' : fmt(row.p_value, 4)}</td><td style={{ padding: 8 }}>{row.q_value_fdr === null ? '—' : fmt(row.q_value_fdr, 4)}</td>
                <td style={{ padding: 8 }}><Badge color={row.significativo_fdr ? S.purple : S.dim}>{row.significativo_fdr ? 'SINAL EXPLORATÓRIO' : row.eventos_independentes < 5 ? 'AMOSTRA INSUFICIENTE' : 'SEM SINAL'}</Badge></td>
              </tr>)}</tbody>
            </table>
          </div>
        </Card>}
        ...
        <Card>
          ...
          <div style={{ display: 'grid', gap: 10, marginTop: 12 }}>
            {reviewQueue.map((event) => {
              ...
              return <article key={event.id} style={{ background: S.soft, border: `1px solid ${reviewedIds.has(event.id) ? S.green : S.border}`, borderRadius: 9, padding: 12 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(145px,1fr))', gap: 8, marginTop: 10 }}>
                  <select value={draft.relevance} onChange={(e) => setDrafts((old) => ({ ...old, [event.id]: { ...draft, relevance: e.target.value } }))} style={{ background: S.panel, color: S.text, border: `1px solid ${S.border}`, borderRadius: 7, padding: 7 }}>
                    {RELEVANCES.map((value) => <option key={value} value={value}>{value}</option>)}
                  </select>
                  <select value={draft.category} onChange={(e) => setDrafts((old) => ({ ...old, [event.id]: { ...draft, category: e.target.value } }))} style={{ background: S.panel, color: S.text, border: `1px solid ${S.border}`, borderRadius: 7, padding: 7 }}>
                    {CATEGORIES.map((value) => <option key={value} value={value}>{value}</option>)}
                  </select>
                  <select value={draft.direction} onChange={(e) => setDrafts((old) => ({ ...old, [event.id]: { ...draft, direction: e.target.value } }))} style={{ background: S.panel, color: S.text, border: `1px solid ${S.border}`, borderRadius: 7, padding: 7 }}>
                    {DIRECTIONS.map((value) => <option key={value} value={value}>{value}</option>)}
                  </select>
                  <input value={draft.asset} onChange={(e) => setDrafts((old) => ({ ...old, [event.id]: { ...draft, asset: e.target.value.toUpperCase() } }))} placeholder="Ativo principal" style={{ background: S.panel, color: S.text, border: `1px solid ${S.border}`, borderRadius: 7, padding: 7 }} />
                </div>
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  <input value={draft.notes} onChange={(e) => setDrafts((old) => ({ ...old, [event.id]: { ...draft, notes: e.target.value } }))} placeholder="Observação opcional" style={{ flex: 1, background: S.panel, color: S.text, border: `1px solid ${S.border}`, borderRadius: 7, padding: 7 }} />
                  <button disabled={!session || savingReview === event.id} onClick={() => void saveReview(event)} style={{ background: S.blue, color: 'white', border: 0, borderRadius: 7, padding: '7px 12px', cursor: session ? 'pointer' : 'not-allowed', opacity: session ? 1 : 0.5 }}>{savingReview === event.id ? 'Salvando…' : reviewedIds.has(event.id) ? 'Atualizar' : 'Revisar'}</button>
                </div>
              </article>;
            })}
          </div>
        ...
        <Card>
          ...
          {events.length === 0 ? <div style={{ color: S.dim }}>A coleta ainda não registrou eventos.</div> : <div style={{ display: 'grid', gap: 10 }}>
            {events.map((event) => <article key={event.id} style={{ background: S.soft, border: `1px solid ${S.border}`, borderRadius: 9, padding: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
                <a href={event.url} target="_blank" rel="noreferrer" style={{ color: S.text, fontSize: 13, fontWeight: 600, textDecoration: 'none', flex: 1 }}>{event.titulo}</a>
