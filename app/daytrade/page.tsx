 ...
 * - Playbook educacional de tendência com rompimento, checklist e semáforo.
 * - Plano de entrada, invalidação, alvo e calculadora de tamanho da posição.
 * - Login com retorno seguro e vínculo com a Central de Oportunidades.
 */
...
const DAYTRADE_RETURN_TARGET = sanitizeRedirectTarget('/daytrade', {
  fallback: '/daytrade',
});
const DAYTRADE_LOGIN_HREF =
  `/alertas?next=${encodeURIComponent(DAYTRADE_RETURN_TARGET)}`;

function buildOpportunityHref(opportunityId: string): string {
  if (!isSafeUuid(opportunityId)) return '/oportunidades';

  return sanitizeRedirectTarget(
    `/oportunidades?focus=${opportunityId}`,
...
function opportunityLifecycleLabel(status: string | null): string {
  const labels: Record<string, string> = {
    pending: 'Pendente',
    under_review: 'Em revisão',
    revalidating: 'Aceita · revalidando',
    invalidated: 'Invalidada',
    expired: 'Expirada',
    rejected: 'Recusada',
    opening: 'Abrindo posição',
    open: 'Posição aberta',
    exit_pending: 'Saída pendente',
    closing: 'Encerrando',
    closed: 'Encerrada',
    error: 'Atenção necessária',
  };

  return status ? labels[status] ?? status.replaceAll('_', ' ') : 'Vinculada';
}

function opportunityLifecycleColor(status: string | null): string {
  if (status === 'open' || status === 'closed') return S.green;
  if (
    status === 'invalidated' ||
    status === 'expired' ||
    status === 'rejected' ||
...
export default function DayTradePage() {
  ...
  // Persistência e execução controlada.
  const [serverSetup, setServerSetup] = useState<ServerSetupRecord | null>(null);
  const [setupOpportunity, setSetupOpportunity] =
    useState<SetupOpportunityRecord | null>(null);
  const [opportunityAction, setOpportunityAction] =
    useState<RemoteActionState>({
      status: 'idle',
      message: '',
    });
  const [serverAction, setServerAction] = useState<RemoteActionState>({
  ...
  useEffect(() => {
    ...
    const { data: subscription } = supabase.auth.onAuthStateChange(
      (_event: string, nextSession: Session | null) => {
        ...
        if (!nextSession) {
          setServerSetup(null);
          setSetupOpportunity(null);
          setOpportunityAction({ status: 'idle', message: '' });
          setTestnetPreview(null);
          setConfirmTestnet(false);
        }
      },
  ...
  const loadOpportunityForSetup = useCallback(async (
    setupId: string,
    hintedOpportunityId?: string | null,
  ): Promise<SetupOpportunityRecord | null> => {
    if (!session || !isSafeUuid(setupId)) {
      setSetupOpportunity(null);
      setOpportunityAction({ status: 'idle', message: '' });
      return null;
    }
    ...
    const safeHint =
      hintedOpportunityId && isSafeUuid(hintedOpportunityId)
        ? hintedOpportunityId
        : null;

    setOpportunityAction({
      status: 'loading',
      message: 'Consultando a Central de Oportunidades...',
    });

    let query = supabase
      .from('trade_opportunities')
      .select(
        'id,user_id,source_id,lifecycle_status,entry_decision,execution_environment,expires_at,detected_at,updated_at',
      )
      .eq('user_id', session.user.id)
      .eq('source_type', 'daytrade_setup')
      .eq('source_id', setupId)
      .eq('opportunity_type', 'entry');

    if (safeHint) {
      query = query.eq('id', safeHint);
    }

    const { data, error: opportunityError } = await query
      .order('detected_at', { ascending: false })
      .limit(1)
      .maybeSingle();

    if (opportunityError) {
      const fallbackRecord: SetupOpportunityRecord | null = safeHint
        ? {
            id: safeHint,
            user_id: session.user.id,
            source_id: setupId,
      ...
      setSetupOpportunity(fallbackRecord);

      const missingCentral =
        opportunityError.code === '42P01' ||
        opportunityError.message.includes('trade_opportunities');

      setOpportunityAction({
        status: 'error',
        message: missingCentral
          ? 'A Central ainda não está disponível no banco. Aplique primeiro a migration de oportunidades.'
          : `O setup foi salvo, mas não foi possível consultar a oportunidade: ${opportunityError.message}`,
      });
    ...
    const opportunity =
      (data as SetupOpportunityRecord | null) ?? null;

    setSetupOpportunity(opportunity);
    setOpportunityAction({
      status: opportunity ? 'success' : 'idle',
      message: opportunity
        ? 'O setup está vinculado à Central de Oportunidades.'
        : 'Nenhuma oportunidade foi criada para este setup.',
    });
  ...
  const persistCurrentSetup = useCallback(async (
    ...
    const saved = data.setup as ServerSetupRecord;
    const automation =
      (data.opportunity ?? null) as OpportunityAutomationResult | null;
    const hintedOpportunityId =
      automation?.opportunity_id &&
      isSafeUuid(automation.opportunity_id)
        ? automation.opportunity_id
        : null;

    setServerSetup(saved);

    let linkedOpportunity: SetupOpportunityRecord | null = null;

    if (hintedOpportunityId) {
      linkedOpportunity = await loadOpportunityForSetup(
        saved.id,
        hintedOpportunityId,
      );
      ...
      setOpportunityAction({
        ...
        message:
          automation.error ??
          'O setup foi salvo, mas a oportunidade não pôde ser criada.',
      });
    } else if (saved.status === 'condicoes_atendidas') {
      // Compatibilidade com uma oportunidade criada pelo sincronizador ou por
      // uma versão anterior da Edge Function.
      linkedOpportunity = await loadOpportunityForSetup(saved.id);
    } else {
      setSetupOpportunity(null);
      setOpportunityAction({
        status: 'idle',
        message:
          'A Central cria uma oportunidade somente quando todas as condições do playbook estão atendidas.',
      });
    }

    if (announce) {
      const setupLabel =
        TREND_BREAKOUT_STATUS_LABELS[saved.status]?.label ??
        saved.status;
      const opportunitySuffix =
        linkedOpportunity || hintedOpportunityId
          ? ' A oportunidade correspondente está disponível na Central.'
          : '';

      setServerAction({
        status: 'success',
        message: `Setup salvo como “${setupLabel}”.${opportunitySuffix}`,
      });
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          {!session ? (
            <a href={DAYTRADE_LOGIN_HREF} style={{ color: S.green, textDecoration: 'none' }}>
              Entrar
            </a>
          ) : (
            <button
        ...
        <Card>
          ...
          >
            <div>
              ...
              <div style={{ color: S.dim, fontSize: 11, marginTop: 3 }}>
                O servidor reavalia o candle encerrado e vincula setups confirmados à Central antes de liberar qualquer prévia.
              </div>
            </div>
            {!session && (
              <a href={DAYTRADE_LOGIN_HREF} style={{ color: S.green, fontSize: 12 }}>
                Entre para usar os recursos conectados
              </a>
            )}
          </div>
          ...
          >
            ...
            >
              ...
              {serverSetup && (
                <div
                  style={{
                    ...
                    border: `1px solid ${
                      setupOpportunity
                        ? `${opportunityLifecycleColor(setupOpportunity.lifecycle_status)}66`
                        : S.border
                    }`,
                    borderRadius: 8,
                    padding: 10,
                    background: setupOpportunity
                      ? `${opportunityLifecycleColor(setupOpportunity.lifecycle_status)}0d`
                      : S.bg,
                    textAlign: 'center',
                  }}
                >
                  ...
                  {setupOpportunity ? (
                    <>
                      <div
                        style={{
                          color: opportunityLifecycleColor(
                            setupOpportunity.lifecycle_status,
                          ),
                          fontSize: 12,
                          fontWeight: 800,
                          marginTop: 4,
                      ...
                      >
                        Ambiente{' '}
                        {setupOpportunity.execution_environment === 'real'
                          ? 'real'
                          : 'Testnet'}
                        {setupOpportunity.expires_at
                          ? ` · expira ${formatDateTime(setupOpportunity.expires_at)}`
                          : ''}
                      </div>

                      <a
                        href={buildOpportunityHref(setupOpportunity.id)}
                        style={{
                          display: 'inline-flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                    ...
                    <div
                      style={{
                        color:
                          opportunityAction.status === 'error'
                            ? S.red
                            : S.dim,
                        fontSize: 10,
                        lineHeight: 1.45,
                    ...
                    >
                      {opportunityAction.status === 'loading'
                        ? 'Consultando vínculo...'
                        : opportunityAction.message ||
                          'O vínculo aparecerá aqui quando o checklist completo gerar uma oportunidade.'}
                    </div>
                  )}
                </div>
              )}
              ...
              {setupOpportunity && opportunityAction.status === 'error' && (
                <div
                  style={{
                    marginTop: 8,
                    color: S.red,
