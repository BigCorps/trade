...
export default function DayTradePage() {
  ...
  const handleRunBacktest = useCallback(async () => {
    ...
    if (!Number.isInteger(candleCount) || candleCount < 350 || candleCount > 1200) {
      setBacktestAction({
        status: 'error',
        message:
          'O backtest rápido aceita entre 350 e 1200 candles. Para períodos maiores ' +
          '(até 730 dias), use a Análise Aprofundada na página Test, que processa em segundo plano.',
      });
      return;
    }
    if (!(initialCapital > 0)) {
