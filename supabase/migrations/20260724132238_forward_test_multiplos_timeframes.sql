...
alter table public.forward_test_config
  add column timeframes text[];

update public.forward_test_config
set timeframes = array['1d', '12h', '4h', '1h']
where ativo;

update public.forward_test_config
set timeframes = array[timeframe]
where timeframes is null;

alter table public.forward_test_config
  alter column timeframes set not null;

alter table public.forward_test_config
  drop column timeframe;

-- O sinal passa a registrar em qual horizonte foi detectado.
...
-- diário e o de 12h das 00:00, por exemplo), então o timeframe entra na chave.
alter table public.forward_test_signals
