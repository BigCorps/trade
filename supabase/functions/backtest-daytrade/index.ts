// ============================================================================
// VigIA Trade — Edge Function: backtest-daytrade
// supabase/functions/backtest-daytrade/index.ts
// ============================================================================
// Busca candles públicos encerrados da Binance e executa o mesmo playbook
// determinístico usado pelo módulo de Day Trade.
//
// Segurança:
// - exige JWT válido;
// - não acessa chaves da Binance;
// - não grava ordens;
// - não executa compras;
// - limita o volume histórico para proteger a Edge Function.
// ============================================================================
...
// ============================================================================
// SEÇÃO 12: handler por quantidade de candles ou período
// ============================================================================

/**
 * Contrato preservado e ampliado:
 *
 * Chamada antiga — permanece válida:
 * {
 *   "symbol": "BTCUSDT",
 *   "timeframe": "15m",
 *   "candle_count": 750,
 ...
 * }
 *
 * Chamada por quantidade:
 * {
 *   "action": "run",
 *   "mode": "single" | "compare_all",
 *   "strategy": "trend_breakout",
 *   "history_mode": "candle_count",
 *   "candle_count": 1000
 * }
 *
 * Chamada por período:
 ...
 *   "mode": "single" | "compare_all",
 *   "strategy": "trend_breakout",
 *   "history_mode": "date_range",
 *   "start_time": "2025-01-01T00:00:00.000Z",
 *   "end_time": "2026-01-01T00:00:00.000Z"
 * }
 *
 * Regras de execução:
 * - até 3.000 candles: execução direta nesta Edge Function;
 * - acima de 3.000 candles: cria uma tarefa em backtest_runs;
 * - o worker processar-backtest-daytrade concluirá tarefas extensas;
 * - ausência de mode continua significando single;
 * - ausência de strategy continua significando trend_breakout;
 * - ausência de history_mode continua usando candle_count;
 * - as respostas individuais antigas continuam com result,
 *   requested_candles, execution_ms e generated_at.
 */

type BacktestRequestMode = 'single' | 'compare_all';
type BacktestHistoryMode = 'candle_count' | 'date_range';

type ParsedStrategyOptionsMap = Partial<DayTradeBacktestStrategyOptionsMap>;

interface ParsedCandleCountHistory {
  mode: 'candle_count';
  candleCount: number;
...
interface ParsedDateRangeHistory {
  mode: 'date_range';
  startTime: number;
  endTime: number;
...
type ParsedBacktestHistory =
  | ParsedCandleCountHistory
  | ParsedDateRangeHistory;

interface QueuedBacktestRun {
  id: string;
  status: string;
...
class BacktestRequestError extends Error {
...
const VALID_VOLATILITY_REGIMES = new Set<VolatilityRegime>([
  'indisponível',
  'calmo',
  'normal',
  'volátil',
...
const DEFAULT_CANDLE_COUNT = 750;
const MIN_CANDLE_COUNT = 350;
const MAX_CANDLE_COUNT = 3_000;
const DIRECT_EXECUTION_CANDLE_LIMIT = 3_000;
const MAX_DATE_RANGE_CANDLES = 220_000;
const MAX_DATE_RANGE_DAYS = 730;
const MAX_BINANCE_PAGE_SIZE = 1_000;
const MAX_DIRECT_BINANCE_PAGES = 10;
...
function optionalInteger(
  ...
  if (!Number.isInteger(parsed)) {
    throw new BacktestRequestError(`${name} deve ser um número inteiro.`);
  }
  return parsed;
}
...
function optionalBoolean(
  ...
  if (typeof value !== 'boolean') {
    throw new BacktestRequestError(`${name} deve ser true ou false.`);
  }

  return value;
}

function parseBlockedVolatilityRegimes(
  value: unknown,
  name: string,
  ...
  if (!Array.isArray(value) || value.length === 0) {
    throw new BacktestRequestError(
      `${name} deve ser uma lista não vazia de regimes.`,
    );
  }

  const regimes = value.map((item) => String(item) as VolatilityRegime);
  const invalid = regimes.find((item) => !VALID_VOLATILITY_REGIMES.has(item));

  if (invalid) {
    throw new BacktestRequestError(
      `${name} contém o regime inválido "${invalid}".`,
    );
  }
...
function parseTrendBreakoutStrategyOptions(
  value: unknown,
  path = 'strategy_options',
): TrendBreakoutOptions {
  const source = isPlainRecord(value) ? value : {};

  return compactObject({
    minimumRelativeVolume: optionalNumber(
      source.minimumRelativeVolume,
      `${path}.minimumRelativeVolume`,
      0.01,
      10,
    ),
    atrStopMultiple: optionalNumber(
      source.atrStopMultiple,
      `${path}.atrStopMultiple`,
      0.05,
      10,
    ),
    minimumRiskRewardRatio: optionalNumber(
      source.minimumRiskRewardRatio,
      `${path}.minimumRiskRewardRatio`,
      0.1,
      20,
    ),
    observationDistanceAtr: optionalNumber(
      source.observationDistanceAtr,
      `${path}.observationDistanceAtr`,
      0,
      20,
    ),
    maximumLateEntryDistanceAtr: optionalNumber(
      source.maximumLateEntryDistanceAtr,
      `${path}.maximumLateEntryDistanceAtr`,
      0,
      20,
    ),
    minimumStopDistanceAtr: optionalNumber(
      source.minimumStopDistanceAtr,
      `${path}.minimumStopDistanceAtr`,
      0.01,
      20,
    ),
    maximumStopDistanceAtr: optionalNumber(
      source.maximumStopDistanceAtr,
      `${path}.maximumStopDistanceAtr`,
      0.02,
      50,
    ),
    blockedVolatilityRegimes: parseBlockedVolatilityRegimes(
...
function parseTrendPullbackStrategyOptions(
  value: unknown,
  path = 'strategy_options',
): TrendPullbackOptions {
  const source = isPlainRecord(value) ? value : {};
  ...
  return compactObject({
    ...
    minimumPreviousCandleVolumeRatio: optionalNumber(
      source.minimumPreviousCandleVolumeRatio,
      `${path}.minimumPreviousCandleVolumeRatio`,
      0.01,
      20,
    ),
    atrStopMultiple: optionalNumber(
    ...
    structuralStopBufferAtr: optionalNumber(
      source.structuralStopBufferAtr,
      `${path}.structuralStopBufferAtr`,
      0,
      10,
    ),
    minimumRiskRewardRatio: optionalNumber(
    ...
    pullbackToleranceAtr: optionalNumber(
      source.pullbackToleranceAtr,
      `${path}.pullbackToleranceAtr`,
      0,
      10,
    ),
    maximumLateEntryDistanceAtr: optionalNumber(
  ...
  }) as TrendPullbackOptions;
}

function parseSqueezeBreakoutStrategyOptions(
  value: unknown,
  path = 'strategy_options',
): SqueezeBreakoutOptions {
  const source = isPlainRecord(value) ? value : {};
  ...
  return compactObject({
    compressionLookback: optionalInteger(
      source.compressionLookback,
      `${path}.compressionLookback`,
      2,
      500,
    ),
    baselineLookback: optionalInteger(
      source.baselineLookback,
      `${path}.baselineLookback`,
      2,
      1_000,
    ),
    maximumRangeContractionRatio: optionalNumber(
      source.maximumRangeContractionRatio,
      `${path}.maximumRangeContractionRatio`,
      0.01,
      10,
    ),
    maximumCompressionRangeAtr: optionalNumber(
      source.maximumCompressionRangeAtr,
      `${path}.maximumCompressionRangeAtr`,
      0.01,
      50,
    ),
    minimumRelativeVolume: optionalNumber(
    ...
    minimumBreakoutDistanceAtr: optionalNumber(
      source.minimumBreakoutDistanceAtr,
      `${path}.minimumBreakoutDistanceAtr`,
      0,
      20,
    ),
    observationDistanceAtr: optionalNumber(
  ...
  }) as SqueezeBreakoutOptions;
}

function parseRangeMeanReversionStrategyOptions(
  value: unknown,
  path = 'strategy_options',
): RangeMeanReversionOptions {
  const source = isPlainRecord(value) ? value : {};
  ...
  return compactObject({
    rangeLookback: optionalInteger(
      source.rangeLookback,
      `${path}.rangeLookback`,
      3,
      2_000,
    ),
    meanLookback: optionalInteger(
      source.meanLookback,
      `${path}.meanLookback`,
      2,
      2_000,
    ),
    maximumSlowEmaSlopeAtrPerCandle: optionalNumber(
      source.maximumSlowEmaSlopeAtrPerCandle,
      `${path}.maximumSlowEmaSlopeAtrPerCandle`,
      0,
      10,
    ),
    maximumEmaSpreadAtr: optionalNumber(
      source.maximumEmaSpreadAtr,
      `${path}.maximumEmaSpreadAtr`,
      0,
      20,
    ),
    minimumRangeWidthAtr: optionalNumber(
      source.minimumRangeWidthAtr,
      `${path}.minimumRangeWidthAtr`,
      0.01,
      100,
    ),
    maximumRangeWidthAtr: optionalNumber(
      source.maximumRangeWidthAtr,
      `${path}.maximumRangeWidthAtr`,
      0.02,
      200,
    ),
    maximumEntryZScore: optionalNumber(
      source.maximumEntryZScore,
      `${path}.maximumEntryZScore`,
      -20,
      20,
    ),
    maximumSupportDistanceAtr: optionalNumber(
      source.maximumSupportDistanceAtr,
      `${path}.maximumSupportDistanceAtr`,
      0,
      20,
    ),
    minimumRelativeVolume: optionalNumber(
    ...
    maximumRelativeVolume: optionalNumber(
      source.maximumRelativeVolume,
      `${path}.maximumRelativeVolume`,
      0.01,
      100,
    ),
    atrStopMultiple: optionalNumber(
  ...
  }) as RangeMeanReversionOptions;
}

function parseStrategyOptionsForId(
  strategyId: DayTradeStrategyId,
  value: unknown,
  path: string,
): DayTradeBacktestStrategyOptionsMap[DayTradeStrategyId] {
  switch (strategyId) {
    case 'trend_breakout':
      return parseTrendBreakoutStrategyOptions(value, path);
    case 'trend_pullback':
      return parseTrendPullbackStrategyOptions(value, path);
    case 'squeeze_breakout':
      return parseSqueezeBreakoutStrategyOptions(value, path);
    case 'range_mean_reversion':
      return parseRangeMeanReversionStrategyOptions(value, path);
  }
}

function parseStrategyOptionsMap(
  value: unknown,
  requestedStrategy: DayTradeStrategyId,
): ParsedStrategyOptionsMap {
  if (value === undefined || value === null) return {};

  if (!isPlainRecord(value)) {
    throw new BacktestRequestError('strategy_options deve ser um objeto.');
  }

  const hasStrategyKeys = DAYTRADE_STRATEGY_IDS.some((id) => id in value);

  if (!hasStrategyKeys) {
    return {
      [requestedStrategy]: parseStrategyOptionsForId(
        requestedStrategy,
        value,
        'strategy_options',
      ),
    } as ParsedStrategyOptionsMap;
  }

  const result: ParsedStrategyOptionsMap = {};

  for (const strategyId of DAYTRADE_STRATEGY_IDS) {
    if (!(strategyId in value)) continue;

    result[strategyId] = parseStrategyOptionsForId(
      strategyId,
      value[strategyId],
      `strategy_options.${strategyId}`,
    ) as never;
  }
...
function parseBacktestOptions(value: unknown): MultiStrategyBacktestOptions {
  if (value === undefined || value === null) return {};
  if (!isPlainRecord(value)) {
    throw new BacktestRequestError('backtest_options deve ser um objeto.');
  }

  const intrabarPriority = value.intrabarPriority;
  if (
    intrabarPriority !== undefined &&
    intrabarPriority !== 'stop_first' &&
    intrabarPriority !== 'target_first'
  ) {
    throw new BacktestRequestError(
      'backtest_options.intrabarPriority deve ser stop_first ou target_first.',
    );
  }

  return compactObject({
    initialCapitalUsdt: optionalNumber(
      value.initialCapitalUsdt,
      'backtest_options.initialCapitalUsdt',
      1,
      1_000_000_000,
    ),
    riskPercent: optionalNumber(
      value.riskPercent,
      'backtest_options.riskPercent',
      0.01,
      2,
    ),
    feeRatePct: optionalNumber(
      value.feeRatePct,
      'backtest_options.feeRatePct',
      0,
      10,
    ),
    slippagePct: optionalNumber(
      value.slippagePct,
      'backtest_options.slippagePct',
      0,
      10,
    ),
    stepSize: optionalNumber(
      value.stepSize,
      'backtest_options.stepSize',
      0.000000000001,
      1_000_000_000,
    ),
    minQuantity: optionalNumber(
      value.minQuantity,
      'backtest_options.minQuantity',
      0,
      1_000_000_000,
    ),
    minNotional: optionalNumber(
      value.minNotional,
      'backtest_options.minNotional',
      0,
      1_000_000_000,
    ),
    maxPositionNotional: optionalNumber(
      value.maxPositionNotional,
      'backtest_options.maxPositionNotional',
      0.01,
      1_000_000_000_000,
    ),
    intrabarPriority: intrabarPriority as BacktestIntrabarPriority | undefined,
    maximumHoldingCandles: optionalInteger(
      value.maximumHoldingCandles,
      'backtest_options.maximumHoldingCandles',
      0,
      1_000_000,
    ),
    maximumNextOpenDistanceAtr: optionalNumber(
      value.maximumNextOpenDistanceAtr,
      'backtest_options.maximumNextOpenDistanceAtr',
      0,
      100,
    ),
  }) as MultiStrategyBacktestOptions;
}

function moneyManagementValue(
...
function parseMoneyManagementOptions(
  value: unknown,
  backtestOptions: MultiStrategyBacktestOptions,
  ...
  if (!isPlainRecord(value)) {
    throw new BacktestRequestError(
      'money_management_options deve ser um objeto.',
    );
  }

  const rawMode = moneyManagementValue(value, 'mode', 'mode');
  ...
  const policyVersionRaw = moneyManagementValue(
    value,
    'policyVersion',
    'policy_version',
  );
  ...
  const parsed: BacktestMoneyManagementOptions = compactObject({
    mode: mode as BacktestPositionSizingMode,
    baseRiskPercent: optionalNumber(
      moneyManagementValue(value, 'baseRiskPercent', 'base_risk_percent'),
      'money_management_options.baseRiskPercent',
      0.01,
      10,
    ),
    minimumRiskPercent: optionalNumber(
      moneyManagementValue(value, 'minimumRiskPercent', 'minimum_risk_percent'),
      'money_management_options.minimumRiskPercent',
      0.01,
      10,
    ),
    maximumRiskPercent: optionalNumber(
      moneyManagementValue(value, 'maximumRiskPercent', 'maximum_risk_percent'),
      'money_management_options.maximumRiskPercent',
      0.01,
      10,
    ),
    winMultiplier: optionalNumber(
      moneyManagementValue(value, 'winMultiplier', 'win_multiplier'),
      'money_management_options.winMultiplier',
      1,
      3,
    ),
    lossMultiplier: optionalNumber(
      moneyManagementValue(value, 'lossMultiplier', 'loss_multiplier'),
      'money_management_options.lossMultiplier',
      0.01,
      1,
    ),
    lossReductionStart: optionalInteger(
      moneyManagementValue(value, 'lossReductionStart', 'loss_reduction_start'),
      'money_management_options.lossReductionStart',
      1,
      10,
    ),
    martingaleLossMultiplier: optionalNumber(
      moneyManagementValue(
        value,
        'martingaleLossMultiplier',
    ...
    maximumMultiplier: optionalNumber(
      moneyManagementValue(value, 'maximumMultiplier', 'maximum_multiplier'),
      'money_management_options.maximumMultiplier',
      1,
      10,
    ),
    martingaleMaximumMultiplier: optionalNumber(
      moneyManagementValue(
        value,
        'martingaleMaximumMultiplier',
    ...
    maximumSequenceSteps: optionalInteger(
      moneyManagementValue(
        value,
        'maximumSequenceSteps',
        'maximum_sequence_steps',
      ),
      'money_management_options.maximumSequenceSteps',
      0,
      10,
    ),
    pauseAfterConsecutiveLosses: optionalInteger(
      moneyManagementValue(
        value,
        'pauseAfterConsecutiveLosses',
    ...
    balanceUsageLimitPct: optionalNumber(
      moneyManagementValue(
        value,
        'balanceUsageLimitPct',
    ...
    compareWithFixed: optionalBoolean(
      moneyManagementValue(value, 'compareWithFixed', 'compare_with_fixed'),
      'money_management_options.compareWithFixed',
    ),
    policyVersion,
  }) as BacktestMoneyManagementOptions;
  ...
  try {
    ...
    throw new BacktestRequestError(
      error instanceof Error
        ? error.message
        : 'Configuração de gerenciamento de risco inválida.',
    );
  }
}
...
function parseComparisonOptions(
  value: unknown,
): CompareDayTradeBacktestsOptions {
  if (value === undefined || value === null) return {};
  if (!isPlainRecord(value)) {
    throw new BacktestRequestError('comparison_options deve ser um objeto.');
  }

  let strategyIds: DayTradeStrategyId[] | undefined;

  if (value.strategyIds !== undefined && value.strategyIds !== null) {
    if (!Array.isArray(value.strategyIds) || value.strategyIds.length === 0) {
      throw new BacktestRequestError(
        'comparison_options.strategyIds deve ser uma lista não vazia.',
      );
    }

    strategyIds = Array.from(new Set(
      value.strategyIds.map((item) => assertDayTradeStrategyId(item)),
    ));
  }

  return compactObject({
    strategyIds,
    minimumTradesForRanking: optionalInteger(
      value.minimumTradesForRanking,
      'comparison_options.minimumTradesForRanking',
      1,
      1_000_000,
    ),
    continueOnStrategyError: optionalBoolean(
      value.continueOnStrategyError,
      'comparison_options.continueOnStrategyError',
    ),
  }) as CompareDayTradeBacktestsOptions;
}

function parseMode(value: unknown): BacktestRequestMode {
  if (value === undefined || value === null || value === '') return 'single';
  if (value === 'single' || value === 'compare_all') return value;

  throw new BacktestRequestError('mode deve ser single ou compare_all.');
}

function parseHistoryMode(value: unknown): BacktestHistoryMode {
  if (value === undefined || value === null || value === '') {
    return 'candle_count';
  }

  if (value === 'candle_count' || value === 'date_range') return value;

  throw new BacktestRequestError(
    'history_mode deve ser candle_count ou date_range.',
  );
}

function parseRequestedStrategy(value: unknown): DayTradeStrategyId {
  if (value === undefined || value === null || value === '') {
    return DEFAULT_DAYTRADE_STRATEGY_ID;
  }

  try {
    return assertDayTradeStrategyId(value);
  } catch (error) {
    throw new BacktestRequestError(
      error instanceof Error ? error.message : 'strategy inválida.',
    );
  }
}

function parseIsoTimestamp(value: unknown, name: string): number {
  if (typeof value !== 'string' || value.trim() === '') {
    throw new BacktestRequestError(`${name} deve ser uma data ISO válida.`);
  }
...
function parseHistory(
...
): ParsedBacktestHistory {
  const historyMode = parseHistoryMode(body.history_mode);

  if (historyMode === 'candle_count') {
    const candleCount = Number(body.candle_count ?? DEFAULT_CANDLE_COUNT);

    if (
      !Number.isInteger(candleCount) ||
      candleCount < MIN_CANDLE_COUNT ||
      candleCount > MAX_CANDLE_COUNT
    ) {
      throw new BacktestRequestError(
        `candle_count deve ser um inteiro entre ${MIN_CANDLE_COUNT} e ${MAX_CANDLE_COUNT}.`,
      );
    }
  ...
  const startTime = parseIsoTimestamp(body.start_time, 'start_time');
  const requestedEndTime = parseIsoTimestamp(body.end_time, 'end_time');
  ...
  const now = Date.now();
  const endTime = Math.min(requestedEndTime, now);

  if (endTime <= startTime) {
    throw new BacktestRequestError(
      'O período informado ainda não possui candles encerrados.',
    );
  }

  const rangeMs = endTime - startTime;
  const rangeDays = Math.ceil(rangeMs / (24 * 60 * 60 * 1_000));

  if (rangeDays > MAX_DATE_RANGE_DAYS) {
    throw new BacktestRequestError(
      `O período máximo permitido é de ${MAX_DATE_RANGE_DAYS} dias.`,
    );
  }

  const estimatedCandles = Math.ceil(
    rangeMs / TIMEFRAME_MILLISECONDS[timeframe],
  );

  if (estimatedCandles < MIN_CANDLE_COUNT) {
    throw new BacktestRequestError(
      `O período possui aproximadamente ${estimatedCandles} candles; o mínimo é ${MIN_CANDLE_COUNT}.`,
    );
  }

  if (estimatedCandles > MAX_DATE_RANGE_CANDLES) {
    throw new BacktestRequestError(
      `O período excede o limite de ${MAX_DATE_RANGE_CANDLES.toLocaleString('pt-BR')} candles estimados.`,
    );
  }
  ...
  return {
    ...
    requestedEndTime,
    estimatedCandles,
    asynchronous: estimatedCandles > DIRECT_EXECUTION_CANDLE_LIMIT,
  };
}
...
function parseKline(row: unknown): DayTradeCandle {
  ...
  if (
    ...
    throw new Error('A Binance retornou um candle com valor inválido.');
  }
...
async function fetchKlinePage(
  ...
  for (const baseUrl of PUBLIC_BASE_URLS) {
    ...
    try {
      ...
      if (!Array.isArray(rows)) {
        errors.push(`${baseUrl}: resposta não é uma lista`);
        continue;
      }
  ...
  throw new Error(`Falha ao consultar Binance: ${errors.join(' | ')}`);
}
...
async function fetchHistoricalCandles(
  ...
  while (
    byOpenTime.size < requestedCount &&
    attempts < MAX_DIRECT_BINANCE_PAGES
  ) {
    attempts += 1;
    const remaining = requestedCount - byOpenTime.size;
    const pageLimit = Math.min(MAX_BINANCE_PAGE_SIZE, remaining + 3);
    const page = await fetchKlinePage(symbol, timeframe, endTime, pageLimit);

    if (page.length === 0) break;

    const now = Date.now();
    for (const candle of page) {
      if (candle.closeTime < now) byOpenTime.set(candle.openTime, candle);
    }

    const earliestOpenTime = Math.min(
      ...page.map((candle) => candle.openTime),
    );

    if (!Number.isFinite(earliestOpenTime) || earliestOpenTime <= 0) break;
  ...
  const normalized = normalizeClosedCandles([...byOpenTime.values()]);
  const selected = normalized.slice(-requestedCount);

  if (selected.length < requestedCount) {
    throw new Error(
      `Foram obtidos ${selected.length} candles encerrados, abaixo dos ${requestedCount} solicitados.`,
    );
  }
...
async function fetchHistoricalCandlesByDateRange(
  symbol: string,
  timeframe: DayTradeIndicatorTimeframe,
  startTime: number,
  endTime: number,
  ...
  const byOpenTime = new Map<number, DayTradeCandle>();
  let cursorEndTime = endTime - 1;
  let attempts = 0;

  while (
    cursorEndTime >= startTime &&
    attempts < MAX_DIRECT_BINANCE_PAGES
  ) {
    attempts += 1;
    ...
    if (!Number.isFinite(earliestOpenTime) || earliestOpenTime <= 0) break;
    if (earliestOpenTime <= startTime) break;

    const nextEndTime = earliestOpenTime - 1;
    if (nextEndTime >= cursorEndTime) break;

    cursorEndTime = nextEndTime;
    if (page.length < MAX_BINANCE_PAGE_SIZE) break;
  }

  const candles = normalizeClosedCandles([...byOpenTime.values()])
    .filter((candle) => candle.openTime >= startTime && candle.openTime < endTime);

  if (candles.length < MIN_CANDLE_COUNT) {
    throw new Error(
      `Foram obtidos ${candles.length} candles encerrados no período; o mínimo é ${MIN_CANDLE_COUNT}.`,
    );
  }

  if (candles.length > DIRECT_EXECUTION_CANDLE_LIMIT) {
    throw new BacktestRequestError(
      'O período ultrapassou o limite da execução direta e deve ser processado de forma assíncrona.',
      409,
    );
  }
...
function legacyTrendBreakoutBacktestOptions(
  options: MultiStrategyBacktestOptions,
...
function buildHistoryResponse(
  history: ParsedBacktestHistory,
  actualCandles?: number,
): Record<string, unknown> {
  if (history.mode === 'candle_count') {
    return {
      mode: history.mode,
      candle_count: history.candleCount,
      estimated_candles: history.estimatedCandles,
      actual_candles: actualCandles ?? history.candleCount,
    };
  }

  return {
    mode: history.mode,
    start_time: toIso(history.startTime),
    end_time: toIso(history.endTime),
    requested_end_time: toIso(history.requestedEndTime),
    end_time_clamped_to_now: history.endTime !== history.requestedEndTime,
    estimated_candles: history.estimatedCandles,
    actual_candles: actualCandles ?? null,
  };
}

function buildNormalizedRequestPayload(
  mode: BacktestRequestMode,
  strategyId: DayTradeStrategyId,
  symbol: string,
  timeframe: DayTradeIndicatorTimeframe,
  history: ParsedBacktestHistory,
  strategyOptions: ParsedStrategyOptionsMap,
  backtestOptions: MultiStrategyBacktestOptions,
  ...
  return {
    ...
    candle_count: history.mode === 'candle_count'
      ? history.candleCount
      : null,
    start_time: history.mode === 'date_range'
      ? toIso(history.startTime)
      : null,
    end_time: history.mode === 'date_range'
      ? toIso(history.endTime)
      : null,
    estimated_candle_count: history.estimatedCandles,
    strategy_options: strategyOptions,
    backtest_options: backtestOptions,
    money_management_options: moneyManagementOptions,
    comparison_options: comparisonOptions,
  };
}

async function queueBacktestRun(
  ...
  symbol: string,
  timeframe: DayTradeIndicatorTimeframe,
  history: ParsedDateRangeHistory,
  strategyOptions: ParsedStrategyOptionsMap,
  backtestOptions: MultiStrategyBacktestOptions,
  moneyManagementOptions: ResolvedBacktestMoneyManagementOptions | null,
  comparisonOptions: CompareDayTradeBacktestsOptions,
): Promise<QueuedBacktestRun> {
  ...
  if (countError) {
    throw new Error(`Não foi possível verificar a fila: ${countError.message}`);
  }

  if ((count ?? 0) >= MAX_ACTIVE_RUNS_PER_USER) {
    throw new BacktestRequestError(
      `Você já possui ${count} análises pendentes ou em processamento. Aguarde uma delas terminar.`,
      429,
    );
  }

  const requestPayload = buildNormalizedRequestPayload(
    mode,
    strategyId,
    symbol,
    timeframe,
  ...
  const { data, error } = await serviceClient
    ...
    .insert({
      ...
      status: 'pending',
      mode,
      strategy: mode === 'single' ? strategyId : null,
      symbol,
      timeframe,
      history_mode: history.mode,
      candle_count: null,
      ...
      end_time: toIso(history.endTime),
      estimated_candles: history.estimatedCandles,
      total_candles: history.estimatedCandles,
      processed_candles: 0,
      progress_pct: 0,
      progress_message: 'Análise criada e aguardando processamento.',
      request_payload: requestPayload,
    })
    .select(
      'id, status, progress_pct, processed_candles, total_candles, created_at',
    )
    .single();

  if (error || !data) {
    throw new Error(
      `Não foi possível criar a análise aprofundada: ${error?.message ?? 'registro não retornado'}`,
    );
  }

  return data as QueuedBacktestRun;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!supabaseUrl || !anonKey) {
      return jsonResponse(req, { error: 'configuração interna ausente' }, 500);
    }
    ...
    if (!isPlainRecord(body)) {
      throw new BacktestRequestError('O corpo deve ser um objeto JSON.');
    }

    const action = String(body.action ?? 'run').trim().toLowerCase();
    if (action !== 'run') {
      throw new BacktestRequestError('action deve ser run.');
    }

    const mode = parseMode(body.mode);
    const strategyId = parseRequestedStrategy(body.strategy);
    const symbol = String(body.symbol ?? 'BTCUSDT').trim().toUpperCase();
    const timeframe = String(
      body.timeframe ?? '15m',
    ) as DayTradeIndicatorTimeframe;

    if (!/^[A-Z0-9]{5,20}$/.test(symbol) || !symbol.endsWith('USDT')) {
      throw new BacktestRequestError(
        'symbol inválido; use um par Spot cotado em USDT',
      );
    }
    ...
    const history = parseHistory(body, timeframe);
    const strategyOptions = parseStrategyOptionsMap(
      body.strategy_options,
      strategyId,
    );
    const backtestOptions = parseBacktestOptions(body.backtest_options);
    const nestedBacktestOptions = isPlainRecord(body.backtest_options)
      ? body.backtest_options
      : null;
    const rawMoneyManagementOptions =
      body.money_management_options ??
      body.position_sizing_options ??
      nestedBacktestOptions?.moneyManagement ??
      nestedBacktestOptions?.money_management;
    const moneyManagementOptions = parseMoneyManagementOptions(
      rawMoneyManagementOptions,
      backtestOptions,
    );
    const comparisonOptions = parseComparisonOptions(body.comparison_options);

    if (history.asynchronous) {
      if (!serviceRoleKey) {
        return jsonResponse(
          req,
          { error: 'SUPABASE_SERVICE_ROLE_KEY não configurada para a fila' },
          500,
        );
      }
      ...
      const run = await queueBacktestRun(
        serviceClient,
        user.id,
        mode,
        strategyId,
      ...
      return jsonResponse(req, {
        ...
        symbol,
        timeframe,
        history: buildHistoryResponse(history),
        money_management: moneyManagementOptions,
        progress_pct: run.progress_pct ?? 0,
        processed_candles: run.processed_candles ?? 0,
        total_candles: run.total_candles ?? history.estimatedCandles,
    ...
    const candles = history.mode === 'candle_count'
      ? await fetchHistoricalCandles(
          symbol,
          timeframe,
          history.candleCount,
        )
      : await fetchHistoricalCandlesByDateRange(
          symbol,
          timeframe,
          history.startTime,
          history.endTime,
    ...
    const requestedCandles = history.mode === 'candle_count'
      ? history.candleCount
      : history.estimatedCandles;

    if (mode === 'compare_all') {
      const baseComparison = compareDayTradeBacktests({
        candles,
        indicatorOptions,
        strategyOptions,
        backtestOptions,
      ...
      const comparison = moneyManagementOptions
        ? rebuildComparisonWithMoneyManagement(
            baseComparison,
            moneyManagementOptions,
          )
        : baseComparison;
      ...
      return jsonResponse(req, {
        ...
        symbol,
        timeframe,
        requested_candles: requestedCandles,
        candle_count: candles.length,
        execution_ms: Date.now() - startedAt,
        generated_at: generatedAt,
        history: buildHistoryResponse(history, candles.length),
        money_management: moneyManagementOptions,
        comparison,
      });
    }
    ...
    /**
     * Preserva exatamente o motor e o formato histórico do trend_breakout.
     * As demais estratégias continuam usando o motor genérico.
     */
    const baseResult = strategyId === TREND_BREAKOUT_STRATEGY_ID
      ? runTrendBreakoutBacktest({
          candles,
          indicatorOptions,
          strategyOptions: strategyOptions.trend_breakout,
          backtestOptions: legacyTrendBreakoutBacktestOptions(backtestOptions),
        })
      : runMultiStrategyBacktest({
          strategyId,
          candles,
    ...
    const result = moneyManagementOptions
      ? enrichBacktestResultWithMoneyManagement(
          baseResult,
          moneyManagementOptions,
        )
      : baseResult;
    ...
    console.error('[backtest-daytrade]', error);
