// ============================================================================
// VigIA Trade — motor compartilhado de backtests aprofundados
// supabase/functions/_shared/daytrade-backtest-engine.ts
// ============================================================================
// Gerado a partir do mesmo motor determinístico usado por backtest-daytrade.
// Não consulta banco, não acessa chaves privadas e não executa ordens.
// ============================================================================
...
function buildHistoricalEvaluations(
  ...
  for (let index = 0; index < candles.length; index++) {
    ...
    const evaluationWindowStart = Math.max(
      0,
      index - full.requiredCandles - 5,
    );

    evaluations[index] = evaluateTrendBreakout({
      candles: candles.slice(evaluationWindowStart, index + 1),
      indicators: snapshot,
...
function buildMultiStrategyHistoricalEvaluations(
  ...
  // Mantém apenas a janela realmente necessária pela estratégia. Isso evita
  // crescimento quadrático em históricos longos sem introduzir lookahead.
  const evaluationWindowSize = Math.max(
    getBacktestStrategyRequiredCandleCount(
      strategyId,
      indicatorOptions,
      strategyOptions,
    ),
    full.requiredCandles,
  ) + 5;
